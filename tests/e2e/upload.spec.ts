...
test.describe("Upload page", () => {
    ...
    test("shows and hides the drag overlay", async ({ page }) => {
        await page.goto("/upload");
        await expect(page.getByRole("button", { name: "+ Add Photos" })).toBeVisible();
        // Drag listeners are attached via useIsomorphicLayoutEffect (fires before paint),
        // so they are guaranteed to be in place before the button assertion resolves.
        // A single dispatch is sufficient; a retry loop would over-increment the drag counter.
